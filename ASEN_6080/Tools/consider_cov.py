...
class ConsiderCov:
    ...
    def time_update(self, x_hat : np.ndarray, P_hat : np.ndarray, S_hat : np.ndarray, theta : np.ndarray, phi : np.ndarray, c : float, P_cc : np.ndarray):
        """
    ...
    def propagate_traj(self, initial_state : np.ndarray, time_vector : np.ndarray, consider_parameters : list):
        ...
        theta_history = np.zeros((state_length, num_consider_params, len(time_vector)))
        for i, augmented_state in enumerate(augmented_state_history):
            ...
            theta_history[:, :, i] = flattened_theta.reshape((state_length, num_consider_params))
    ...
    def run(self, initial_state : np.ndarray, initial_P : np.ndarray, consider_parameters : list, initial_S : np.ndarray, initial_theta : np.ndarray, c : float, P_cc : np.ndarray, time_vector : np.ndarray, measurement_data : pd.DataFrame):
        ...
        # Build psi_history
        for i, (stm, theta) in enumerate(zip(stm_history, theta_history)):
        ...
        return state_history, stm_history, theta_history, measurement_residuals_matrix, H_x_matrix, H_c_matrix
