...
lkf_estimated_state_history, lkf_covariance_history = lkf.run(initial_state_guess, np.zeros(7), P_0, measurement_data, R=np.diag(noise_var), max_iterations=1, apply_smoothing=True)
batch_estimated_state, batch_estimated_covariance = batch_estimator.estimate_initial_state(initial_state_guess, measurement_data, np.diag(noise_var), tol=2e-9, a_priori_covariance=P_0, max_iterations=1)